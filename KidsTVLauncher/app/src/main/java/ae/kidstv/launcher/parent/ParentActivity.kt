...
import ae.kidstv.launcher.BuildConfig
...
import ae.kidstv.launcher.ui.ChannelGridActivity
...
class ParentActivity : AppCompatActivity() {
    ...
    private fun render() {
        ...
        addInfo("Kids TV ${BuildConfig.VERSION_NAME}")
        ...
        addButton("Refresh all playlists") {
            lifecycleScope.launch {
                val n = (application as KidsTvApp).catalogRepository.refreshAllPlaylists(force = true)
        ...
        addButton("Export catalog JSON") {
            lifecycleScope.launch {
                val json = (application as KidsTvApp).catalogRepository.exportJson()
    ...
    private fun addChannelRow(channel: ContentChannel) {
        ...
        row.addView(btn("Refresh") {
            lifecycleScope.launch {
                val r = (application as KidsTvApp).catalogRepository.refreshChannelFromYoutube(channel.id)
    ...
    private fun promptPlaylist(channel: ContentChannel) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Save") { _, _ ->
                lifecycleScope.launch {
                    (application as KidsTvApp).catalogRepository.setPlaylistId(
                        channel.id,
                        input.text.toString()
                    )
                    reload()
    ...
    private fun promptVideoIds(channel: ContentChannel) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Add") { _, _ ->
                lifecycleScope.launch {
                    (application as KidsTvApp).catalogRepository.addManualVideoIds(
                        channel.id,
                        input.text.toString()
                    )
                    reload()
    ...
    private fun promptDirect(channel: ContentChannel) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Add") { _, _ ->
                ...
                lifecycleScope.launch {
                    (application as KidsTvApp).catalogRepository.addDirectVideo(
                        channel.id,
                        title.text.toString(),
                        u
                    )
    ...
    private fun update(transform: (CatalogSettings) -> CatalogSettings) {
        lifecycleScope.launch {
            (application as KidsTvApp).catalogRepository.update(transform)
            reload()
        }
    }
